<?php
    ...
    if(isset($_SESSION['a']))
    {
		...
		$farmerid=$_SESSION['profile_ID'];
		
		//to get date
		$now = new DateTime();
		$closing_date = $now->modify('+1 day')->format('Y-m-d H:i:s');
		...
		$sql3="INSERT INTO auctiondetails (P_ID,Bid,Status,Closing_Date) VALUES ('$pid','$price','0','$closing_date')";
		...
		$sql="INSERT INTO product(Name, Type, Stock, Price, Description, Image, Farmer_ID, AFlag) VALUES ('$pname', '$type', '$stock', '$price', '$des', '$img1', '$farmerid','0')";
		if (mysqli_query($con, $sql))
		{
		header("location: LandingPage.php");
		}
...
<div class="container-fluid" style="margin-top:40px;">
  <div class="row">
    ...
    <div class="col-4">
      ...
      <button type="submit" name="auction" class="text-white bg-indigo-600 border-0 py-2 px-6 mr-6 focus:outline-none hover:bg-red-500 rounded" style="margin-top:15px;">Auction</button>
      <button type="submit" name="sell" class="text-white bg-green-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded" style="margin-top:15px;">Sell</button>
    </div>
    <div  class="col-2">
      </div>
  </div>
